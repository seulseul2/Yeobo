...
public class AttractionRepository2 {
    ...
    public List<AttractionResponseDto> searchAttractionList(String name,long userId){ // score 다시 가져오기
        String sql = "SELECT a.attraction_id,a.name,s.score,a.image FROM attraction a left join score s on s.user_id = :userId and a.attraction_id = s.attraction_id and a.name LIKE :name "+
