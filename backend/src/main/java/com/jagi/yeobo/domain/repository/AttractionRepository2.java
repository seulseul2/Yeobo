...
public class AttractionRepository2 {
    ...
    public List<AttractionScoreDto> findAllByUserId(long userId){
        List<Object[]> scores = em.createQuery("SELECT s.userId.id, s.score, s.attractionId.id,a.name,a.image FROM Score as s join Attraction as a on s.attractionId.id = a.id WHERE s.userId.id = :userId")
                .setParameter("userId",userId).getResultList();
        List<AttractionScoreDto> scoreDtoList = new ArrayList<>();

        for (Object[] s:scores) {
            AttractionScoreDto scoreSave = AttractionScoreDto.builder()
                    .userId(Long.valueOf(String.valueOf(s[0])))
                    .score(Double.valueOf(String.valueOf(s[1])))
                    .attractionId(Long.valueOf(String.valueOf(s[2])))
                    .name(String.valueOf(s[3]))
                    .img(String.valueOf(s[4]))
                    .build();
